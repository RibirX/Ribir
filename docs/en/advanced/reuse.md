...
fn generate_new_item() -> GenWidget {
    GenWidget::new(|| @Text { text: "I am a new instance!" }.into_widget())
}
...
fn dynamic_list() -> Widget<'static> {
    ...
    fn_widget! {
        ...
        @ReuseScope {
            @Column {
                @ {
                    pipe!($items.clone()).map(move |list| {
...
When a global key is matched, the fallback child block (`@AudioPlayerWidget {}` above) will simply be ignored and will not participate in building, preventing unnecessary performance overhead.
...
@ReuseScope {
    defs: [
        // Register early
        reuse_def(header.clone(), || @Text { text: "Persistent Header" }.into_widget())
    ],
    @Column {
