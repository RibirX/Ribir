...
- 当调用 `animate.run()` 时，动画开始跟踪帧更新
- 在每一帧中，动画计算当前进度并使用 `shallow()` 临时更新绑定状态以通过反应绑定传播值
- 所有注册的帧订阅者（如 `pipe!` 表达式）都会被通知并更新它们的视觉属性
- 当动画完成或停止时，恢复原始状态
...
fn_widget! {
    ...
    let animate = @Animate {
        state: opacity_state.clone_writer(), // 动画化独立状态
        ...
    };

    @SizedBox {
        opacity: pipe!(*$read(opacity_state)), // SizedBox 的 opacity 在动画期间不会更新！
...
fn_widget! {
    let w = @SizedBox {
        opacity: pipe!(*$read(opacity_state)), // 管道在动画期间会更新！
        ...
    };
    let animate = @Animate {
        state: w.opacity(),
...
`Animate` 提供了最基础的能力，让您可以手动控制动画的开始和停止。除此，Ribir 还提供了一种简便的方式，让你将动画绑定到属性本身，当属性值发生变化时自动触发过渡动画。
...
这里 `w.opacity()` 返回的 `StateWriter`，实现了 `AnimateState` trait，通过 `transition()` 方法设置动画属性，当通过 `StateWriter` 对值进行修改时自动触发动画的运行。
...
fn delayed_animation_example() -> Widget<'static> {
    fn_widget! {
        ...
        let animate = @Animate {
            ...
            .delay(Duration::from_millis(500)) // 开始前等待 500ms
            .box_it()
        };
