...
Ribir 提供了一个强大的内置属性系统，让您可以为任何 Widget 添加常用功能，如布局控制（margin、alignment）、视觉效果（background、border、opacity、transform）和交互事件（on_tap、on_hover）。这些功能并非由每个 Widget 单独实现，而是通过 `FatObj` 统一提供。
...
为了支持上述过程，Ribir 提供了 `#[derive(Declare)]` 宏，使 Widget 能够在 DSL 中通过 `@` 语法声明。库中大部分组件都是通过该宏实现的。该宏会自动为您的 Widget 生成必要的代码，包括：

*   创建一个 **Builder**（例如 `Text` 的 `TextBuilder`）。
*   为您的 Widget 实现 **`Declare` trait**，将其与构建器关联。
*   为每个字段生成 **`with_xxx` 方法**，让您能够流畅地设置值。
*   为 Builder 实现 **`ObjDeclarer`**，处理最终的构建步骤 `finish()` 并返回 `FatObj<Stateful<T>>`（其中 `T` 是正在构建的 Widget 类型）。
...
1.  **按需初始化**: `FatObj` 在内部维护所有内置属性（如 `margin`、`padding` 等）的状态，但它们默认为空。只有在您显式使用某个属性时，相关的 Widget 才会被初始化。这确保了未使用的功能不会产生额外的性能开销。
...
大多数情况下，Widget 都是用 `#[derive(Declare)]` 宏定义的。这意味着您可以在使用 `@` 语法声明 Widget 时，直接使用内置属性。
...
16. `margin`
17. `cursor`
18. **事件** (`mix_builtin`: `on_tap`, `on_pointer_move` 等)
19. `transform`
20. `opacity`
21. `visibility`
22. `disabled`
...
*   **事件包含边距**: 由于**事件**包装在**边距**外层，默认情况下 Widget 的交互区域包括其边距。
...
fn dynamic_opacity_example() -> Widget<'static> {
    fn_widget! {
        ...
        @Container {
            size: Size::new(80., 20.),
            // 容器的背景色始终根据文本颜色的变化而变化。
            background: pipe!($read(w.foreground()).clone()).map(|c| {
                match c {
                    Brush::Color(c) => invert_filter(0.4).apply_to(&c).into(), // 使用反色滤镜使颜色反色。
                    _ => c,
                }
            }),
            // 在事件处理程序中修改组件的属性
...
1.  `$write(w.foreground())`
    *   `w.foreground()` 返回前景色属性的 `StateWriter`。
    *   `$write(...)` 在该写入器上提供对实际 `foreground` 值的可变访问。
2.  `pipe!(*$read(w.foreground()))`
    *   `w.foreground()` 返回前景色属性的 `StateWriter`。
    *   `pipe!` 将写入器的使用与其状态分离，然后订阅状态变化。
    *   `$read(...)` 在该写入器上访问实际 `foreground` 值的引用。

此模式适用于所有内置属性（例如 `background()`、`margin()` 等）。**请注意**，您必须使用方法（例如 `foreground()`）来获取写入器，而不是直接访问字段，因为字段是私有的。
