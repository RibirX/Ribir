...
pub enum LeadingWidget {
  Text(WidgetPair<Leading, State<Label>>),
  Icon(WidgetPair<Leading, NamedSvg>),
  // Avatar(WidgetPair<Leading, Avatar>),
  // Image(WidgetPair<Leading, Image>),
  // Poster(WidgetOf<Leading>),
  Custom(WidgetOf<Leading>),
}

#[derive(Template)]
pub enum TrailingWidget {
  Text(WidgetPair<Trailing, State<Label>>),
  Icon(WidgetPair<Trailing, NamedSvg>),
  // Avatar(WidgetPair<Trailing, Avatar>),
  // Image(WidgetPair<Trailing, Image>),
  // Poster(WidgetOf<Trailing>),
  Custom(WidgetOf<Trailing>),
}
    ...
    widget! {
      ...
      init ctx => {
        ...
        let line_height = line_height
          .map_or(font_size, |line_height| FontSize::Em(line_height))
          .into_pixel();
              ...
              match leading {
                LeadingWidget::Icon(w) => widget! {
                  DynWidget {
                    ...
                    Icon {
                      size: icon.size,
                      DynWidget::from(w.child)
                    }
                  }
                },
                LeadingWidget::Text(w) => widget! {
                  states { label: w.child.into_readonly() }
                  DynWidget {
