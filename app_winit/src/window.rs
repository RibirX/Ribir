...
use ribir_core::widget_tree::WidgetTree;
use ribir_painter::{Point, Size};

use crate::{event_dispatcher::Dispatcher, from_cursor_icon::RCursorIcon, prelude::Application};
...
impl RawWindow for winit::window::Window {
  ...
  fn set_cursor(&mut self, cursor: CursorIcon) {
    self.set_cursor_icon(RCursorIcon::from(cursor).into())
  }
...
impl Window {
    ...
    P: PainterBackend + 'static,
  {
    ...
    let dispatcher = Dispatcher::new(wnd_ctx.focus_mgr.clone() /* , DispatchInfo::default() */);
...
mod tests {
  use super::*;
  // use crate::test::*;
  use ribir_core::test::{assert_layout_result, ExpectRect, MockBox};
