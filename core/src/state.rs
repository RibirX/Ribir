...
pub trait StateReader: 'static {
  ...
  #[inline]
  fn part_reader<U: ?Sized, F>(&self, map: F) -> MapReader<Self::Reader, F>
  where
...
pub trait StateWatcher: StateReader {
  ...
  fn part_watcher<U: ?Sized, F>(&self, map: F) -> Watcher<MapReader<Self::Reader, F>>
  where
    F: Fn(&Self::Value) -> PartRef<U> + Clone,
    Self: Sized,
  {
...
pub trait StateWriter: StateWatcher {
  type State: StateValue<Value = Self::Value> + Clone
  where
    Self: Sized;
  ...
  ///      @ {
  ///        pipe!($items.len())
  ///          .value_chain(|f| f.filter(|(info, _)| !info.is_from_partial()).box_it())
  ///          .map(move|cnt| {
...
impl<T: 'static> StateWatcher for State<T> {
  ...
  {
    match self.0.into_inner() {
      InnerState::Data(d) => Ok(StateValueReader(StateAtom::from_state_cell(d))),
...
impl<T: 'static> StateWriter for State<T> {
  ...
  where
    ...
    Self: Sized,
  {
    self.as_stateful().part_writer(id, part_map)
  }
}
...
impl<'a, W: ?Sized> WriteRef<'a, W> {
  fn into_value_mut_ref(self) -> ValueMutRef<'a, W> {
    assert!(!self.modified);

    let borrow = self.value.borrow.clone();
    match self.value.inner {
      InnerPart::Ref(inner) => {
        let inner = InnerPart::Ref(inner);
        ValueMutRef { inner, borrow }
      }
      _ => unreachable!(),
    }
  }
...
impl<'a, W: ?Sized> Drop for WriteRef<'a, W> {
  fn drop(&mut self) {
    ...
    if info.has_modified() && !modify_scope.is_empty() {
      info.modified(*modify_scope);

      let info = info.clone();
      let _ = AppCtx::spawn_local(async move {
        info.submit_modified();
      });
    } else {
      info.modified(*modify_scope);
    }
...
impl<V: ?Sized + 'static> StateWriter for Box<dyn StateWriter<Value = V>> {
  type State = WriterAsStateValue<Self>;
  #[inline]
  ...
  where
    ...
    Self: Sized,
  {
    let info = self.write().info.clone();
    let data = StateValue::map(WriterAsStateValue { writer: self.clone_writer() }, part_map);
    let w = StateValueWriter { data, info };
    if let Some(id) = id { w.partial(id) } else { w }
  }
}

#[derive(Debug, Clone)]
pub struct WriterAsStateValue<W> {
  writer: W,
}

impl<T: ?Sized, W> StateValue for WriterAsStateValue<W>
where
  W: StateWriter<Value = T>,
{
  type Value = T;

  fn read(&self) -> ReadRef<Self::Value> { self.writer.read() }
  ...
  fn write(&self) -> ValueMutRef<T> {
    let w = self.writer.write();
    w.into_value_mut_ref()
  }

  fn clone_box(&self) -> Box<dyn StateValue<Value = Self::Value>> {
    Box::new(WriterAsStateValue { writer: self.writer.clone_writer() })
  }
}

impl<V: ?Sized + 'static> Clone for WriterAsStateValue<Box<dyn StateWriter<Value = V>>> {
  fn clone(&self) -> Self { WriterAsStateValue { writer: self.writer.clone_writer() } }
}
