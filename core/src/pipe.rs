...
pub(crate) trait InnerPipe: Pipe {
    ...
    Self::Value: 'static,
  {
    ...
    let pipe_node = PipeNode::share_capture(w.id(), Box::new(id_share.clone()), ctx);
    ...
    let u = modifies.subscribe(move |(_, w)| {
      handle.with_ctx(|ctx| {
        ...
        if !ctx.window().is_in_another_regenerating(id) {
          ...
          pipe_node.remove_logic_child_infos(&id_share);
          pipe_node
            .infos_iter()
            .for_each(|i| i.single_replace(id, new_id));
    ...
    Self: Sized,
  {
    ...
    // first widget -- if parent is also a multi pipe, it will attach a
    // `DynWidgetInfo` to the first widget.
    let pipe_node = PipeNode::share_capture(first, Box::new(ids_share.clone()), ctx);
    for w in widgets.iter().skip(1) {
      let arena = &ctx.tree.borrow().arena;
      w.id()
        .assert_get(arena)
        .query_most_outside(|node: &PipeNode| node.push_info(ids_share.dyn_clone()));
    }
    ...
    *ids_share.borrow_mut() = widgets.iter().map(|w| w.id()).collect();
    ...
    let u = modifies.subscribe(move |(_, m)| {
      handle.with_ctx(|ctx| {
        let old = ids_share.borrow().clone();
        ...
        if !ctx.window().is_in_another_regenerating(old[0]) {
          ...
          pipe_node.remove_logic_child_infos(&ids_share);
          pipe_node
            .infos_iter()
            .for_each(|i| i.multi_replace(&old, &new));
          old[1..].iter().for_each(|w| {
            w.assert_get(&ctx.tree.borrow().arena)
              .query_most_outside(|node: &PipeNode| node.extend_infos(pipe_node.infos_iter()));
          });
  ...
  where
    Self: Sized,
  {
    fn on_pipe_without_primary(
      range: &RangeInclusive<WidgetId>,
      ctx: &BuildCtx,
      f: impl Fn(&PipeNode),
    ) {
      let arena = &ctx.tree.borrow().arena;
      for w in range.end().ancestors(arena) {
        if &w == range.start() {
          break;
        }
        w.assert_get(arena).query_most_outside(&f);
      }
    }
    ...
    let pipe_node = PipeNode::share_capture(p.id(), Box::new(id_share.clone()), ctx);
    let range = half_to_close_interval(p.id()..child, ctx);
    on_pipe_without_primary(&range, ctx, |node| node.push_info(id_share.dyn_clone()));

    let c_pipe_node = pipe_node.clone();
    *id_share.borrow_mut() = range;

    let handle = ctx.handle();

    let u = modifies.subscribe(move |(_, w)| {
      handle.with_ctx(|ctx| {
        let (top, bottom) = id_share.borrow().clone().into_inner();
        ...
        if !ctx.window().is_in_another_regenerating(top) {
          ...
          pipe_node
            .infos_iter()
            .for_each(|i| i.single_range_replace(&(top..=bottom), &new_rg));

          on_pipe_without_primary(&new_rg, ctx, |node| {
            node.extend_infos(pipe_node.infos_iter())
          });
...
struct InnerPipeNode {
  data: Box<dyn Render>,
  infos: SmallVec<[Box<dyn DynWidgetInfo>; 1]>,
}
trait DynWidgetInfo: Any {
  ...
  fn ptr_eq(&self, other: &dyn DynWidgetInfo) -> bool;
  fn dyn_clone(&self) -> Box<dyn DynWidgetInfo>;
  fn as_any(&self) -> &dyn Any;
}

macro_rules! impl_trivial_methods {
  () => {
    fn ptr_eq(&self, other: &dyn DynWidgetInfo) -> bool {
      other
        .as_any()
        .downcast_ref::<Self>()
        .map_or(false, |other| Sc::ptr_eq(self, other))
    }

    fn dyn_clone(&self) -> Box<dyn DynWidgetInfo> { Box::new(self.clone()) }
...
impl PipeNode {
  fn share_capture(id: WidgetId, info: Box<dyn DynWidgetInfo>, ctx: &BuildCtx) -> Self {
    let tree = &mut ctx.tree.borrow_mut().arena;
    let mut pipe_node = id
      .assert_get(tree)
      .query_most_outside(|node: &PipeNode| node.clone());
    if pipe_node.is_none() {
      id.wrap_node(tree, |r| {
        let inner_node = InnerPipeNode { data: r, infos: SmallVec::new() };
    ...
    // Safety: init before.
    let node = unsafe { pipe_node.unwrap_unchecked() };
    node.push_info(info);
    node
  }
  ...
  fn primary_transplant(&self, old: WidgetId, new: WidgetId, ctx: &BuildCtx) {
    let mut t = ctx.tree.borrow_mut();
    new.assert_get(&t.arena).query_most_outside(|n: &PipeNode| {
      n.degenerate().for_each(|info| self.front_push_info(info));
    });

    let [old_node, new_node] = t.get_many_mut(&[old, new]);
    std::mem::swap(&mut self.as_mut().data, new_node);
    std::mem::swap(old_node, new_node);
  }

  /// remove dynamic widget information until to `to`, and return the rest.
  fn remove_logic_child_infos(&self, to: &dyn DynWidgetInfo) {
    let infos = &mut self.as_mut().infos;
    if infos.len() > 1 {
      let idx = infos.iter().position(|info| info.ptr_eq(to)).unwrap();
      infos.drain(..idx);
    } else {
      // if the `infos` only have one element, that must be the `to` itself.
      assert!(infos[0].ptr_eq(to))
    }
  }

  fn infos_iter(&self) -> impl Iterator<Item = &Box<dyn DynWidgetInfo>> {
    self.as_ref().infos.iter()
  }

  fn front_push_info(&self, info: Box<dyn DynWidgetInfo>) { self.as_mut().infos.insert(0, info); }

  fn extend_infos<'i>(&self, iter: impl Iterator<Item = &'i Box<dyn DynWidgetInfo>>) {
    self
      .as_mut()
      .infos
      .extend(iter.map(|info| info.dyn_clone()));
  }

  fn push_info(&self, info: Box<dyn DynWidgetInfo>) { self.as_mut().infos.push(info); }

  fn pop_info(&self) { self.as_mut().infos.pop(); }

  // let's the `PipeNode` as a normal `Render` widget and can't be query, return
  // all `DynWidgetInfo`.
  fn degenerate(&self) -> impl Iterator<Item = Box<dyn DynWidgetInfo>> + '_ {
    self.as_mut().infos.drain(..)
  }
  ...
  /// `PipeNode` without `DynWidgetInfo` is a normal `Render` widget. For
  /// example, called `degenerate`.
  fn is_pipe_node(&self) -> bool { !self.as_ref().infos.is_empty() }
}
...
impl Query for PipeNode {
  fn query_inside_first(
    ...
    if self.as_ref().data.query_inside_first(type_id, callback)
      && type_id == TypeId::of::<PipeNode>()
      && self.is_pipe_node()
    {
      callback(self)
    } else {
  ...
  fn query_outside_first(
    ...
    if type_id == TypeId::of::<PipeNode>() && self.is_pipe_node() && !callback(self) {
      return false;
    }

    self.as_ref().data.query_outside_first(type_id, callback)
  }
}
