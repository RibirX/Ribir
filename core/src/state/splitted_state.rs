...
use rxrust::prelude::{BoxIt, ObservableExt};
use std::{
  cell::{Cell, RefMut},
  time::Instant,
};
  ...
  fn raw_modifies(&self) -> CloneableBoxOp<'static, ModifyScope, std::convert::Infallible> {
    ...
    impl<R: StateReader> CloneableChecker for StampCheck<R> {
      ...
      fn box_clone(&self) -> Box<dyn CloneableChecker> {
        Box::new(StampCheck {
          stamp: self.stamp,
          reader: self.reader.clone_reader(),
        })
      }
    }
    ...
    let checker: Box<dyn CloneableChecker> = Box::new(StampCheck {
      stamp: create_at,
      reader: origin.clone_reader(),
    });
