...
pub fn multi_derive(input: &mut syn::DeriveInput) -> TokenStream {
  let info = ProxyDeriveInfo::new(input, "MultiChildWidget", PROXY_PATH)
  ...
  match info {
    Ok(info) => {
      let generics = info
        .attr_fields
        .proxy_bounds_generic(quote! {MultiChildWidget});
