...
use crate::{
  CliCommand,
  util::cargo_settings::{
    CargoPackageSettings, CargoSettings, get_cargo_metadata, get_workspace_dir,
  },
};
...
fn bundle_setting_from_config(
  ...
  let work_space = CargoSettings::load(&work_space_path).ok();
  ...
  Ok(BundleSettings {
    ...
    license_file: config.license_file.map(|l| config_dir.join(l)),
    updater: None,
    ..Default::default()
  })
}
...
impl Bundle {
  fn bundle(&self) -> Result<()> {
    let path = if self.config.is_absolute() {
      self.config.clone()
    } else {
      let root_path = env::current_dir();
      root_path.unwrap().join(self.config.clone())
    };
    let bundle_config: BundleConfig = serde_json::from_reader(File::open(&path)?)?;

    let config_dir = path.parent().unwrap();
    let cargo_settings = CargoSettings::load(&env::current_dir()?)?;
    ...
    let package_types = bundle_config
      ...
      .collect();
    let bundle_setting =
      bundle_setting_from_config(bundle_config, config_dir, cargo_package_settings)?;
